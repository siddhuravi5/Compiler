...
ClassDefBlock : CLASS ClassDefList ENDCLASS	{virtualFuncTableInit(targetFile,dataFile);}
                |
	;
...
ClassDef      : Cname '{'DECL Fieldlists MethodDecl ENDDECL MethodDefns '}'
	...
	| Cname '{'DECL Fieldlists ENDDECL '}'
	;
Cname : ID	{Cinstall($<c>1,NULL);classname=$<c>1;}
	| ID EXTENDS ID	{Cinstall($<c>1,$<c>3);classname=$<c>1;}
	;
...
GdeclBlock : DECL GdeclList ENDDECL {updateBinding();sptop=getSP();printGsymbolTable(); Lsymbolroot=NULL;reset_fnargumentsbinding();}
	| DECL ENDDECL		{}
	;
...
/*param can be empty*/
ParamList2: ParamList			{Linstall("self2",NULL);Linstall("self",NULL);}
	;
